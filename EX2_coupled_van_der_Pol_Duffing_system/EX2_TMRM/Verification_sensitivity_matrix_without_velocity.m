...
clc;clear;close all;
global total_time step_size time_data num_degrees_freedom num_harmonics
...
parameter_a(1,1)=w0;
parameter_a(2, 1) = 0.342172594555505;parameter_a(2, 3) = 0.72594555505;
parameter_a(3:end,:)=[C0_1,S0_1,C0_2,S0_2];   % 6行2列

%% 计算残差
residual=calculate_residual_even_without_velocity(parameter_a);
%% 绘制残差曲线
% figure;
...
%     end
%     temp_real_w0=zeros(2,2*num_degrees_freedom);
%     sensitivity_parameter_a=[temp_real_w0;sensitivity_parameter_a];
%     parameter_a=parameter_a+ddt*sensitivity_parameter_a;% 增量加上去了
% 
%     residual1=calculate_residual_even_without_velocity(parameter_a);
%     aaaa=(residual1-residual)/ddt;
%     figure;
%     plot(time_data,residual(:,num_degrees_freedom*(i+1)+1),'r-')
%     hold on
%     plot(time_data,residual(:,num_degrees_freedom*(i+1)+2),'r-')
%     hold on
% 
...
for i=1:1:num_degrees_freedom
    ...
    sensitivity_parameter_a=zeros(num_harmonics+2,2*num_degrees_freedom);
    sensitivity_parameter_a(2,2*i-1)=1;
    parameter_a=parameter_a+ddt*sensitivity_parameter_a;% 增量加上去了

    residual1=calculate_residual_even_without_velocity(parameter_a);
    ss=residual1-residual;
    aaaa=(residual1-residual)/ddt;
    figure;
    plot(time_data,residual(:,num_degrees_freedom*(2 * num_harmonics * num_degrees_freedom+i+1)+1),'r-')
    hold on
    plot(time_data,residual(:,num_degrees_freedom*(2 * num_harmonics * num_degrees_freedom+i+1)+2),'r-')
    hold on
