...
num_harmonics = 25;
% alpha1=1; alpha2=1; mu=0.8; k=3; alpha=0.6; beta=0.05; epsilon=1.2;
alpha1=1.5; alpha2=0.9; mu=0.8; k=3; alpha=0.8; beta=0.05; epsilon=1.2;

mass_matrix=[1,0;0,1];damping_matrix=[-mu 0;-k alpha+k];stiffness_matrix=[1+k -k;0 -1];

initial_frequency = 1.353;
initial_frequency_new=initial_frequency/2;
% initial_frequency = 0.79;

% Define time parameters
total_time = 2 * pi / initial_frequency_new;
step_size = 2 * pi / (initial_frequency_new * 500);
...
% %x1
harmonic_coefficients(2, :) = [-0.331   0 -0.354   0];
harmonic_coefficients(3, :) = [0.0827   0.109 0.0774   0.1125]; % Initial values for the first-order harmonics
load para_alpha_0.8_period_2.mat;
harmonic_coefficients(1, 1) = harmonic_coefficients(1, 1)*2;
...
for iteration_index = 1:max_iterations_rs
    ...
    residual_identification = calculate_residual_even_double_fre(harmonic_coefficients);
    ...
    for trust_index = 1:max_iterations_tr
        ...
        step_size = 2 * pi / (initial_frequency_new * 500);
        time_data = (0:step_size:total_time);
        residual_updates = calculate_residual_even_double_fre(harmonic_coefficients);
...
final_residual = calculate_residual_even_double_fre(harmonic_coefficients);
...
figure;
plot(displacement(1, :), velocity(1, :), 'b-', 'LineWidth', 1);
legend_handle = legend('$$x_1$$');
set(legend_handle, 'Interpreter', 'latex', 'FontSize', 15);
set(gca, 'FontName', 'Times New Roman', 'FontSize', 15, 'LineWidth', 1.5);
