...
function residual = calculate_residual_even_without_velocity(harmonic_params)
...
for dof_index = 1:num_degrees_freedom
    for harm_index = 1:num_harmonics
        ...
        Dx_alpha2(dof_index,:)=Dx_alpha2(dof_index,:)+initial_frequency^alpha2*(Y1_N_harm_alpha2*harmonic_parameters(harm_index,2*dof_index)-Y2_N_harm*harmonic_parameters(harm_index,2*dof_index-1))*sin(harmonic_term)+...
            initial_frequency^alpha2*(Y1_N_harm_alpha2*harmonic_parameters(harm_index,2*dof_index-1)+Y2_N_harm*harmonic_parameters(harm_index,2*dof_index))*cos(harmonic_term);
...
% for dof_index = 1
%     for harm_index = 1:num_harmonics
%         velocity_dof1(dof_index, :) = velocity_dof1(dof_index, :) + initial_frequency * harm_index  * harmonic_parameters(harm_index, 2 * dof_index);
%     end
% end
...
for dof_index = 1:num_degrees_freedom
    for harm_index = 1:num_harmonics
        ...
        Y1_N_harm_alpha1=harm_index^alpha1*cos(beta2*pi/2);Y2_N_harm_alpha1=harm_index^alpha1*sin(beta2*pi/2);
        Dx_alpha1_w0(dof_index,:)=Dx_alpha1_w0(dof_index,:)+initial_frequency^alpha1*frequency_derivative*(-Y1_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index)+Y2_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index-1)).*sin(harm_index*initial_frequency*time_data)+...
        alpha1*initial_frequency^(alpha1-1)*(Y1_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index)-Y2_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index-1)).*cos(harm_index*initial_frequency*time_data)+...
        +initial_frequency^alpha1*frequency_derivative*(-Y1_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index-1)-Y2_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index)).*cos(harm_index*initial_frequency*time_data)+...
        alpha1*initial_frequency^(alpha1-1)*(-Y1_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index-1)-Y2_N_harm_alpha1*harmonic_parameters(harm_index,2*dof_index)).*sin(harm_index*initial_frequency*time_data);

        Y1_N_harm_alpha2=harm_index^alpha2*cos(alpha2*pi/2);Y2_N_harm=harm_index^alpha2*sin(alpha2*pi/2);
        Dx_alpha2_w0(dof_index,:)=Dx_alpha2_w0(dof_index,:)+initial_frequency^alpha2*frequency_derivative*(Y1_N_harm_alpha2*harmonic_parameters(harm_index,2*dof_index)-Y2_N_harm*harmonic_parameters(harm_index,2*dof_index-1)).*cos(harm_index*initial_frequency*time_data)+...
            alpha2*initial_frequency^(alpha2-1)*(Y1_N_harm_alpha2*harmonic_parameters(harm_index,2*dof_index)-Y2_N_harm*harmonic_parameters(harm_index,2*dof_index-1)).*sin(harm_index*initial_frequency*time_data)+...
            -initial_frequency^alpha2*frequency_derivative*(Y1_N_harm_alpha2*harmonic_parameters(harm_index,2*dof_index-1)+Y2_N_harm*harmonic_parameters(harm_index,2*dof_index)).*sin(harm_index*initial_frequency*time_data)+...
            alpha2*initial_frequency^(alpha2-1)*(Y1_N_harm_alpha2*harmonic_parameters(harm_index,2*dof_index-1)+Y2_N_harm*harmonic_parameters(harm_index,2*dof_index)).*cos(harm_index*initial_frequency*time_data);
...
residual(num_degrees_freedom + 1:2 * num_degrees_freedom,:) = mass_matrix * acceleration_freq_sensitivity + frac_damping_matrix * velocity_freq_sensitivity + stiffness_matrix * displacement_freq_sensitivity...
...
for i = 1:2 * num_harmonics * num_degrees_freedom
    ...
    for dof_index = 1:num_degrees_freedom
        for harm_index = 1:num_harmonics
            ...
            Y1_N_harm_alpha2=harm_index^alpha2*cos(alpha2*pi/2);Y2_N_harm=harm_index^alpha2*sin(alpha2*pi/2);
            Dx_alpha2_a(dof_index,:)=Dx_alpha2_a(dof_index,:)+initial_frequency^alpha2*(Y1_N_harm_alpha2*harmonic_sensitivity(harm_index,2*dof_index)-Y2_N_harm*harmonic_sensitivity(harm_index,2*dof_index-1))*sin(harmonic_term)+...
                initial_frequency^alpha2*(Y1_N_harm_alpha2*harmonic_sensitivity(harm_index,2*dof_index-1)+Y2_N_harm*harmonic_sensitivity(harm_index,2*dof_index))*cos(harmonic_term);
    ...
    residual(num_degrees_freedom * (i + 1) + 1:num_degrees_freedom * (i + 1)+ 2, :) = mass_matrix * acceleration_harm_sensitivity + frac_damping_matrix * velocity_harm_sensitivity + stiffness_matrix * displacement_harm_sensitivity...
    ...
    %     for dof_index = 1
    %         for harm_index = 1:num_harmonics
    %             velocity_harm_sensitivity_dof1(dof_index, :) = velocity_harm_sensitivity_dof1(dof_index, :) + initial_frequency * harm_index * harmonic_sensitivity(harm_index, 2 * dof_index);
    %         end
    %     end
    %
...
for dof = 1:num_degrees_freedom
    ...
    residual(num_degrees_freedom*(2 * num_harmonics * num_degrees_freedom+1+dof)+1:num_degrees_freedom*(2 * num_harmonics * num_degrees_freedom+1+dof)+2,:) = mass_matrix * acceleration_sensitivity_constant + frac_damping_matrix * velocity_sensitivity_constant + stiffness_matrix * displacement_sensitivity_constant...
