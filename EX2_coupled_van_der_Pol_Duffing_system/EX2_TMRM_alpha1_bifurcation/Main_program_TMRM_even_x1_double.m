...
initial_frequency=1.37;
harmonic_coefficients = zeros(num_harmonics + 2, 2 * num_degrees_freedom);
harmonic_coefficients(1, 1) = initial_frequency;

% %x1
harmonic_coefficients(2, :) = [-0.1877   0 -0.1877   0];
harmonic_coefficients(3, :) = [1.119   -0.0545 0.12   -0.0484]; % Initial values for the first-order harmonics

%% 
% % period -2
% load parameter_alpha_0.8_alpha1_1.5_x2_single.mat;

% load parameter_alpha_0.8_alpha1_1.5_x1_single.mat;

% harmonic_coefficients(3,:)=10e5*harmonic_coefficients(3,:);

% load parameter_alpha_0.8_alpha1.55_x1_double;
% harmonic_coefficients(3, 1) = 0.15;
...
figure;
plot(displacement(2, :), velocity(2, :), 'k-', 'LineWidth', 1);
legend_handle = legend('$$x_1$$');
set(legend_handle, 'Interpreter', 'latex', 'FontSize', 15);
set(gca, 'FontName', 'Times New Roman', 'FontSize', 15, 'LineWidth', 1.5);
