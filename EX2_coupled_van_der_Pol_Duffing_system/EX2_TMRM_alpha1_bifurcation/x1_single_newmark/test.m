...
load matlab_1.031_green_num.mat;
num_degrees_freedom = 2;
num_harmonics = 25;
index_bifurcation=2;
x_cal=x_cal';
x=x_cal(1:2,:);dx=x_cal(3:4,:);ddx=x_cal(5:6,:);

x(1,1)=x(1,end);dx(1,1)=dx(1,end);ddx(1,1)=ddx(1,end);
x(2,1)=x(2,end);dx(2,1)=dx(2,end);ddx(2,1)=ddx(2,end);

x_cal=[x;dx;ddx];
x_cal=x_cal';
num_xcal(1).x_cal=x_cal;
num_xcal(1).alpha1=1.031;

for alpha1=1.032:0.001:1.035
    alpha1
    ...
    num_xcal(index_bifurcation).x_cal=x_cal;
    num_xcal(index_bifurcation).alpha1=alpha1;
    index_bifurcation=index_bifurcation+1;
    x(1,1)=x(1,end);dx(1,1)=dx(1,end);ddx(1,1)=ddx(1,end);
    x(2,1)=x(2,end);dx(2,1)=dx(2,end);ddx(2,1)=ddx(2,end);

end
save('x1_num_xcal_1.021_1.031_green.mat', 'num_xcal','-v7.3');
