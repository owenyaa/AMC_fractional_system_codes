...
% ����
mu=0.8;k=3;epsilon=1.2;
% alpha=0.8;
alpha1=1.5;alpha2=0.9;beta=0.05;
...
num_harmonics = 20;
index_bifurcation=1;
for alpha=0.6:0.002:0.9
    load 1_2_x2_inter_para_alpha1_1.5_alpha2_0.9_doublecycle.mat;% x1_period-2
    ...
    total_time =800;% 4 * pi / initial_frequency;
    ...
    num_x_cal(index_bifurcation).x_cal=x_cal;
    index_bifurcation=index_bifurcation+1;
    % mm=1;
    % figure;
    % subplot(2,1,1);
    % plot(Tdata(1:mm:end),x_cal(1:mm:end-1,1),'k-','LineWidth',1.5);
    % hold on;
    ...
    %
    % subplot(2,1,2);
    % plot(Tdata(1:mm:end),x_cal(1:mm:end-1,2),'k-','LineWidth',1.5);
    % hold on;
    % plot(time_data,displacement(2,:),'r-','LineWidth',1.5);
...
% set(gca,'FontName','宋体','FontSize',15,'LineWidth',1.5);
% %相图;
% figure
